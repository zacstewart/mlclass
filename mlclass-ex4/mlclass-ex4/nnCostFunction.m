...
%
temp_y = zeros(num_labels, size(y,1));
for i=1:size(y,1),
  temp_y(y(i), i) = 1;
end;

h = zeros(num_labels, size(X,1));
for i=1:m,
  a1 = [ 1 X(i,:) ];
  z2 = Theta1 * a1';
  a2 = [ 1; sigmoid(z2) ];
...
J = (1/m) * sum(sum((-temp_y' .* log(h)') - (1 - temp_y)' .* log(1 - h)')) + ((lambda/(2*m)) * (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2))));
